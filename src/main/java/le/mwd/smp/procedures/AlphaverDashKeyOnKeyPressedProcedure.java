...
public class AlphaverDashKeyOnKeyPressedProcedure {
	...
	public static void executeProcedure(Map<String, Object> dependencies) {
		...
		double velocity = 0;
		double yaw = 0;
		double miniDashDivider = 0;
		yaw = (entity.rotationYaw);
		pitch = (entity.rotationPitch);
		velocity = 7;
		miniDashDivider = 20;
		if ((world.getWorldInfo().getDayTime() <= LeMwdSmpModVariables.MapVariables.get(world).AlphaverDashKeyLastPressTime
				|| world.getWorldInfo().getDayTime() > LeMwdSmpModVariables.MapVariables.get(world).AlphaverDashKeyLastPressTime + 30)
				&& !(new Object() {
			...
			entity.setMotion(
					(Math.sin((yaw / 180) * Math.PI) * (-1) * Math.cos((pitch / 180) * Math.PI)
							* (Math.abs(entity.getMotion().getX()) + velocity / miniDashDivider)),
					(Math.sin((pitch / 180) * Math.PI) * (Math.abs(entity.getMotion().getY()) + miniDashDivider / 10) * (-1)),
					(Math.cos((yaw / 180) * Math.PI) * Math.cos((pitch / 180) * Math.PI)
							* (Math.abs(entity.getMotion().getZ()) + miniDashDivider / 10)));
		...
		} else if ((world.getWorldInfo().getDayTime() <= LeMwdSmpModVariables.MapVariables.get(world).AlphaverDashKeyLastPressTime
